...
export default function TrabajoSettings({ companies, workCenters, trabajo }: Props) {
    ...
    const [errors, setErrors] = useState<{ company_id?: string; work_center_id?: string }>({});
    const [recentlySuccessful, setRecentlySuccessful] = useState(false);
    const [processing, setProcessing] = useState(false);

    const filteredWorkCenters = companyId === 'none'
        ? workCenters
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        if (companyId === 'none') newErrors.company_id = 'La empresa es obligatoria.';
        if (workCenterId === 'none') newErrors.work_center_id = 'El centro de trabajo es obligatorio.';
        ...
        const payload: Record<string, string | number | null> = {
            ...
            work_center_id: workCenterId !== 'none' ? Number(workCenterId) : null,
        };
                ...
                <div className="space-y-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="grid gap-2">
                            <Label>Empresa</Label>
                            <Select value={companyId} onValueChange={handleEmpresaChange}>
                        ...
                        <div className="grid gap-2">
                            ...
                            <Select
                                ...
                                onValueChange={(value) => {
                                    ...
                                    setErrors((prev) => ({ ...prev, work_center_id: undefined }));
                                }}
                                disabled={filteredWorkCenters.length === 0}
                            >
                                <SelectTrigger className={errors.work_center_id ? 'border-destructive' : ''}>
                            ...
                            {errors.work_center_id && (
                                <p className="text-sm text-destructive">{errors.work_center_id}</p>
                            )}
                        </div>
                        ...
                        <div className="grid gap-3">
                            ...
                            <div className="overflow-hidden rounded-lg border">
                                <table className="w-full text-sm">
                                    ...
                                    <tbody className="divide-y">
                                        {DIAS.map((dia) => (
                                            <tr key={dia.key}>
                                                <td className="px-4 py-2 font-medium">{dia.label}</td>
                                                <td className="px-4 py-2 text-right">
                                                    <input
                                                        ...
                                                        className="w-20 rounded-md border bg-background px-2 py-1 text-right text-sm focus:outline-none focus:ring-1 focus:ring-ring"
                                                        placeholder="0"
                                                    />
                                                </td>
                                            </tr>
                            ...
                            <p className="text-xs text-muted-foreground">
                                Introduce las horas diarias previstas (p. ej. 8, 7.5). Se usa para calcular las horas extra.
                            </p>
                        </div>
